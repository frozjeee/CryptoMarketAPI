...
async def transactionOrders(kafkaSettings: kafkaConfig.Settings = kafkaConfig.getSettings()):
    consumer = AIOKafkaConsumer(kafkaSettings.ORDER_FULFILL_TOPIC,
    ...
    try:
        async for msg in consumer:
            ...
            userBuyWalletQuery = Wallet.select().where(Wallet.c.user_id==matchedOrders.buyOrder.user_id) \
            ...
            userSellWalletQuery = Wallet.select().where(Wallet.c.user_id==matchedOrders.sellOrder.user_id) \
            ...
            try:
                if not matchedOrders.type in ["buy, sell"]:
                    sellUpdateOrderQuery = Order.update() \
                ...
                buyQuantityUpdateQuery = \
                    ...
                    .where(Wallet.c.currency_id==matchedOrders.buyOrder.currency_id) \
                    .values(quantity=userBuyWallet.quantity - matchedOrders.sellOrder.quantity)

                sellQuantityUpdateQuery = \
                    Wallet.update() \
                    .where(Wallet.c.user_id==matchedOrders.sellOrder.user_id) \
                    .where(Wallet.c.currency_id==matchedOrders.sellOrder.currency_id) \
                    .values(quantity=userSellWallet.quantity + matchedOrders.sellOrder.quantity)
